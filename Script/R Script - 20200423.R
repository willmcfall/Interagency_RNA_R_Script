...
data$REFUGEE_STATUS [data$Nationality == ("JOR")] <- "Jordanian"
...
sink("Outputs/Aggregated Analysis Output.txt")
 ...
 ## creates frequency table for all categorical variables
 ...
 ## creates mean, median, standard deviation, etc. for all numeric variables
 ...
 ## creates frequency plots for all categorical variables
 for (i in categorical_var) {
   plot_name = paste(i, " Plot")
   freq_file_name= paste("Graphs/Frequency/",plot_name,".jpg")
   jpeg(freq_file_name)
   PlotXTabs(data, i, i, plottype = "stack")
   dev.off()
 }
 
 
 ## creates histogram for all numeric variables
 for (i in numerical_var) {
   title = paste(i, " Statistics")
   chart_title = paste(i, " Frequency")
   plot_name = paste(i, " Plot")
   a <- as.double(data[,i])
   freq_file_name= paste("Graphs/Frequency/",plot_name,".jpg")
   jpeg(freq_file_name)
   hist(a, col="#19486a", main = chart_title, border = "white", xlab=i)
   dev.off()
 }
...
# Loop through combinations of numerical data and disaggregated data, running means
for (i in numerical_var) {
  for (j in disaggregate_var) {
    title = paste("Mean ",i," by ",j)
    print("######################################################")
    print(title)
}
}
